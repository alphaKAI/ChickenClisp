module orelang.operator.FilterOperator;
...
class FilterOperator : IOperator {
  /**
   * call
   */
    ...
    if (eargs1.type == ValueType.Array) {
      ...
      if (efunc.type == ValueType.Closure) {
        ...
        foreach (elem; array) {
          ...
          if (efunc.getClosure.eval([elem]).getBool) {
            ret ~= elem;
          }
        }
        ...
        foreach (elem; array) {
          ...
          if (efunc.getIOperator.call(engine, [elem]).getBool) {
            ret ~= elem;
          }
        }
